...
# graph 1 geom_density of ozone divided by year (distinguish between years by adding transparency to the geom_density layer).
  EPAair %>%
  ggplot(aes(x = Ozone, fill = factor(Year))) +
  geom_density(alpha = 0.5) + 
  labs(
    title = "Distribution of Ozone Levels by Year",
    x = "Ozone AQI Value",
    y = "Density",
    fill = "Year"
  ) +
...
# graph 2 geom_boxplot of ozone divided by year. Add letters representing a significant difference between 2018 and 2019 (hint: stat_summary). 
EPAboxplot <-
...
# graph 3 geom_violin of ozone divided by year, with the 0.5 quantile marked as a horizontal line. Add letters representing a significant difference between 2018 and 2019. 

EPAviolin <-
  ggplot(EPAair, aes(x = as.factor(Year), 
                     y=Ozone)) +
  geom_violin(draw_quartiles=0.5) + 
  stat_summary(geom = "text",
               fun = max,
               vjust = -1,
               size = 4, 
...
PM2.5.Ozone.regression <- lm(
  data = EPAair,
  PM2.5 ~ Ozone)
summary(PM2.5.Ozone.regression)

# R-squared value:  0.1314, p-value: < 2.2e-16 we need to add mroe variables to this regression
...
PM2.5byOzonePlot <- 
  ...
  geom_text(
    x=100,
    y=75,
    label=expression("PM2.4=0.38*Oxone+15.64")
  )

print(PM2.5byOzonePlot)
...
corrplot(EPAairCorr, emthod= "ellipse")
corrplot.moxed(EPAairCorr, upper = "ellipse")
