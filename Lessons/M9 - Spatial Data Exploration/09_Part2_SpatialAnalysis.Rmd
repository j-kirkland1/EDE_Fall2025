...
ggplot() +
  geom_sf(data = huc8_sf, aes(fill = ACRES), color = NA) +
  scale_fill_viridis_c(name = "Area (acres)") +
  labs(title = "NC HUC8 Watersheds (colored by acreage)") +
  theme_minimal()
...
NC_Dataset <- st_read(here("Data/Spatial/NCHUC8.shp"))

upper_neuse <- NC_Dataset %>%
...
windenergy <- read.csv(
  ...
  mutate(
  HUC_12 = as.factor(HUC_12),
  HUC_8 = as.factor(substr(as.factor(HUC_12), 1, 8))
  )
  

huc8_sf <- huc8_sf %>%
  mutate(HUC_8 = as.factor(HUC_8))

windenergy_huc8 <- windenergy %>%
  group_by(HUC_8) %>%
  summarize(total_wind = sum(AvgWindEnergy, na.rm = TRUE)) %>%
  select(HUC_8, total_wind)

#Join to HUC_8 features
huc8_joined <- left_join(huc8_sf, windenergy_huc8, by = "HUC_8")


#View the outputs

mapview(huc8_joined, zcol = "mean_wind", layer.name = "Average Wind Energy (HUC8)")
...
tri_hucs_select <- huc8_sf_utm %>% 
  st_filter(triCo, .predicate = st_intersects)

#Intersect the HUC_8s
tri_hucs_intersect <- huc8_sf_utm %>% 
  st_intersection(triCo)


mapview(triCo, col.regions = 'blue', alpha.regions = 0.3) + mapview(tri_hucs_intersect, col.regions = 'red')
...
theSite_buffer_30km <- st_buffer(theSite_sfc_transformed, 30000)

bufferMask <- st_intersects(counties_sf_utm, 
                              theSite_buffer_30km, 
                              sparse = FALSE)


counties_within_30km <- counties_sf[bufferMask, ]

mapView(counties_within_30km) + mapView(theSite_buffer_30km)
